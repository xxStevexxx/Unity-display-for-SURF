...
public sealed class PiperGameCameraLayout : MonoBehaviour
{
    ...
    private void RemoveInsetCameras()
    {
        RemoveInsetCamera("Camera X View", ref xCamera);
        RemoveInsetCamera("Camera Y View", ref yCamera);
        RemoveInsetCamera("Camera Z View", ref zCamera);
    }

    private void RemoveInsetCamera(string cameraName, ref Camera camera)
    {
        Transform child = camera != null ? camera.transform : transform.Find(cameraName);
        camera = null;
        if (child == null)
            return;
